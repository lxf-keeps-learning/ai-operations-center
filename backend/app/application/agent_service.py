...
class AgentService:
    ...
    async def stream_events(self, trace_id: str, session_id: str | None = None):
        ...
        yield StreamEvent(event="progress", traceId=trace_id, stage="query_tool", message="正在读取页面上下文")
        await asyncio.sleep(0.1)
        yield StreamEvent(event="progress", traceId=trace_id, stage="analyze", message="正在生成分析摘要")
        ...
        yield StreamEvent(event="done", traceId=trace_id, status="success", message="analysis finished")
    ...
    def _mock_answer_chunks(self, session: SessionRecord) -> list[str]:
        ...
        return [
            ...
            "- 后续可在 Application 层接入 LangGraph，并在 Tool Center 中访问 IOC 业务数据。\n\n",
            "建议下一步补充真实 Tool、Prompt 版本管理、Trace 入库和权限上下文。",
        ]
