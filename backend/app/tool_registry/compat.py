...
def _execute_legacy(
    ...
    execution = tool.run(BaseToolInput(context=context, filters=arguments))
...
async def aexecute_tool(
    ...
    """异步兼容入口，供 Graph 保留取消传播与 Deadline 预算。"""
    if settings.tool_registry_mode == "legacy":
        tool_name = CAPABILITY_TO_LEGACY_NAME.get(name_or_capability, name_or_capability)
        tool = get_tool(tool_name)
        execution = tool.run(BaseToolInput(context=context, filters=arguments))
    ...
    if hasattr(gateway, "aexecute"):
        return await gateway.aexecute(
            capability,
            arguments,
            context,
            confirmation_token=confirmation_token,
