...
工具发现与治理由 MySQL 持久化配置驱动：Agent/MCP/API 按 `capability` 请求能力，`DatabaseToolRegistry` 解析版本（稳定/灰度），`ToolGateway` 统一执行权限、限流、动作确认与审计。旧工具名通过兼容适配器映射，`TOOL_REGISTRY_MODE=legacy` 可随时回退。
