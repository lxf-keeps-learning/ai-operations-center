...
@pytest.mark.anyio
async def test_domain_agent_uses_domain_prompts_and_preserves_result_fields(monkeypatch):
    calls = []

    async def fake_reason(state, **kwargs):
        calls.append(("reason", kwargs["prompt_name"]))
        state["reason_analysis"] = "设备分析结果"
        return state

    async def fake_advice(state, **kwargs):
    ...
    result = await run_domain_agent(state, DOMAIN_AGENT_SPECS["maintenance"])
