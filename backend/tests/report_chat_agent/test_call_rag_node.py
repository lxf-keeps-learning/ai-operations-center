...
class TestCallRagNode:
    ...
    @pytest.mark.anyio
    async def test_skips_when_need_rag_false(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        result = await call_rag_node(state)
    ...
    @pytest.mark.anyio
    async def test_skips_when_rag_query_empty(self) -> None:
    ...
    async def test_returns_results(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        async def fake_retrieve(_req: object):
    ...
    @pytest.mark.anyio
    async def test_empty_results_does_not_fabricate(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    @pytest.mark.anyio
    async def test_failure_does_not_crash(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """RAG 服务失败 → Graph 不崩溃，errors 记录。"""
        async def fake_retrieve(_req: object):
            return _fake_failure_response()
    ...
    @pytest.mark.anyio
    async def test_rag_success_with_one_result(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """RAG 返回单条结果。"""
        async def fake_retrieve(_req: object) -> RagSearchResponse:
            return RagSearchResponse(
                success=True,
                results=[
                    RagSearchResult(
    ...
    @pytest.mark.anyio
    async def test_used_rag_false_when_service_unavailable(self, monkeypatch: pytest.MonkeyPatch) -> None:
