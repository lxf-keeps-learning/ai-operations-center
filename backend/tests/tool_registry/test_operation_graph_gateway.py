...
@pytest.mark.anyio
async def test_query_node_uses_capabilities(monkeypatch) -> None:
    calls: list[str] = []
    ...
    monkeypatch.setattr(node_module, "aexecute_tool", fake_execute)

    await node_module.query_operation_data_node(_safety_state())
...
@pytest.mark.anyio
async def test_query_node_marks_caller_as_internal(monkeypatch) -> None:
    contexts: list[ToolContext] = []

    async def fake_execute(capability, arguments, context, confirmation_token=None, stable_only=False):
...
@pytest.mark.anyio
async def test_query_node_uses_first_http_role(monkeypatch) -> None:
    contexts: list[ToolContext] = []
    ...
    await node_module.query_operation_data_node(state)
...
@pytest.mark.anyio
async def test_query_node_evidence_uses_selected_tool_key(monkeypatch) -> None:
...
@pytest.mark.anyio
async def test_query_node_skips_failed_tools_and_records_errors(monkeypatch) -> None:
    async def fake_execute(capability, arguments, context, confirmation_token=None, stable_only=False):
